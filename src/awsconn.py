...
class awsConn (object):
    ...
    def create_ec2_conn_singapore():
        """ Gets the list of all the available regions and creates an ec2 connection in Singapore Region """ 
        singapore = boto.ec2.regions()[4];
        return boto.ec2.connect_to_region (region_name=singapore.name);

    @staticmethod
    def create_vpc_conn_singapore():
        """  Gets the list of all the available regions and create a VPC connection in Singapore Region """
        singapore = boto.ec2.regions()[4];
