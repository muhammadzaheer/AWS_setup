...
class awsCoreRelease (object):
    ...
    def __init__ (self, conf_file):
        try:
            ...
            print "Conf.json does not contain key " + e.args[0];
    ...
    def __release (self):
        self.ec2_conn.delete_security_group (group_id = self.conf["sg_private"]);
        self.ec2_conn.disassociate_address (association_id = self.conf["eip_assoc"]);
        self.ec2_conn.release_address (allocation_id = self.conf["eip_alloc"]);
        self.__terminate_nat_instance();
        self.ec2_conn.delete_security_group (group_id = self.conf["sg_nat"]);
        self.vpc_conn.disassociate_route_table(self.conf["custom_rtable_assoc"]);
        self.vpc_conn.delete_route (self.conf["custom_rtable"], "0.0.0.0/0");
        self.vpc_conn.delete_route_table (self.conf["custom_rtable"]);
        self.vpc_conn.detach_internet_gateway (self.conf["ig"], self.conf["vpc"]);
    ...
    def __terminate_nat_instance (self):
        self.ec2_conn.terminate_instances (instance_ids = [self.conf["nat_instance"]]);
    
        while True:
            reservation = self.ec2_conn.get_all_instances (instance_ids = 
                                                        [self.conf["nat_instance"]]);
            if reservation[0].instances[0].state != 'terminated':
                print "instance {} shutting-down...".format(reservation[0].instances[0].id);
                time.sleep(10);
            else:
                return;
